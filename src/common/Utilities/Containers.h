#ifndef NCORE_CONTAINERS_H
#define NCORE_CONTAINERS_H
...
{
    template<class T>
    constexpr T* AddressOrSelf(T* ptr) { return ptr; }

    template<class T>
    constexpr T* AddressOrSelf(T& not_ptr) { return std::addressof(not_ptr); }
...
namespace Acore::Containers
{
     ...
     */
    template<class C>
    auto SelectRandomContainerElement(C const& container) -> std::add_const<decltype(*std::begin(container))>::type&
    {
        auto it = std::begin(container);
        std::advance(it, urand(0, static_cast<uint32>(std::size(container)) - 1));
        return *it;
    }
     ...
     */
    template<class C, class Predicate>
    auto SelectRandomContainerElementIf(C const& container, Predicate&& predicate) -> decltype(std::begin(container))
    {
