        ...
        case CHAT_MSG_WHISPER:
            {
                ...
                bool senderIsPlayer = !IsStaff();
                bool receiverIsPlayer = receiver ? !receiver->GetSession()->IsStaff() : true;
                ...
                if (!receiver || (lang != LANG_ADDON && !receiver->isAcceptWhispers() && !receiver->IsInWhisperWhiteList(sender->GetGUID())))
                {
                    SendPlayerNotFoundNotice(to);
                    return;
                }
                ...
                if (receiver->GetLevel() < sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ) ||
                    (!sender->isAcceptWhispers() && !sender->IsInWhisperWhiteList(receiver->GetGUID())))
