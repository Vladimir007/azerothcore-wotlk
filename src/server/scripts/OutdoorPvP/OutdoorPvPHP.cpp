...
void OPvPCapturePointHP::ChangeState()
{
    ...
    switch (_oldState)
    {
        ...
        case OBJECTIVESTATE_ALLIANCE:
            ...
            _pvp->GetMap()->SendZoneText(OutdoorPvPHPBuffZones[0], sObjectMgr->GetNcoreString(HP_LANG_LOSE_A[m_TowerType]).c_str());
            break;
        case OBJECTIVESTATE_HORDE:
            ...
            _pvp->GetMap()->SendZoneText(OutdoorPvPHPBuffZones[0], sObjectMgr->GetNcoreString(HP_LANG_LOSE_H[m_TowerType]).c_str());
            break;
        ...
        case OBJECTIVESTATE_ALLIANCE:
            {
                ...
                if (alliance_towers < 3)
                    ((OutdoorPvPHP*)_pvp)->SetAllianceTowersControlled(++alliance_towers);
                _pvp->GetMap()->SendZoneText(OutdoorPvPHPBuffZones[0], sObjectMgr->GetNcoreString(HP_LANG_CAPTURE_A[m_TowerType]).c_str());
                break;
            }
        case OBJECTIVESTATE_HORDE:
            {
                ...
                if (horde_towers < 3)
                    ((OutdoorPvPHP*)_pvp)->SetHordeTowersControlled(++horde_towers);
                _pvp->GetMap()->SendZoneText(OutdoorPvPHPBuffZones[0], sObjectMgr->GetNcoreString(HP_LANG_CAPTURE_H[m_TowerType]).c_str());
                break;
            }
        case OBJECTIVESTATE_NEUTRAL_ALLIANCE_CHALLENGE:
        case OBJECTIVESTATE_NEUTRAL_HORDE_CHALLENGE:
