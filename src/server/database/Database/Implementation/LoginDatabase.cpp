...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_LOGON_CHALLENGE,
        "SELECT id, UPPER(username), is_active, is_staff, is_superuser, salt, verifier FROM accounts WHERE UPPER(username) = UPPER($1)", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_SEL_RECONNECT_CHALLENGE,
        "SELECT id, UPPER(username), is_active, is_staff, is_superuser, session_key FROM accounts WHERE UPPER(username) = UPPER($1) AND session_key IS NOT NULL", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_SEL_ACCOUNT_INFO_BY_NAME,
        "SELECT id, session_key, is_active, is_staff, is_superuser, locale, total_time FROM accounts WHERE UPPER(username) = UPPER($1) AND session_key IS NOT NULL", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_GET_ACCOUNT_IS_SUPERUSER, "SELECT is_superuser FROM accounts WHERE id=$1", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_GET_ACCOUNT_IS_STAFF, "SELECT is_staff FROM accounts WHERE id=$1 and is_active=TRUE", CONNECTION_SYNCH);
...
LoginDatabaseConnection::~LoginDatabaseConnection() = default;
